<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="tit1">
        <el-button @click="AddFenlei" size="small" type="primary" icon="el-icon-plus">添加岗位分类</el-button>
      </div>
      <div class="myTable">
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.jobcategorylist({
    ...
    async changeKG(row) {
      console.log(row);
      const res = await this.$api.edit_jobcategory(
        {
          ...row,
    ...
    async tabDel(row) {
      console.log(row);
      const res = await this.$api.del_jobcategory({ id: row.id });
      if (res.result == 1) {
        this.$message({
          message: res.msg,
          type: "success"
    ...
    AddOnSubmit(formName) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          if (this.isAdd) {
            // 新增
            const res = await this.$api.add_jobcategory({
              ...this.addForm,
