<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="tit1">
        <el-button @click="AddSaishi" size="small" type="primary" icon="el-icon-plus">添加问题</el-button>
      </div>
      <div class="myTable">
        <vxe-table :loading="loading" align="center" :data="tableData">
          <vxe-table-column field="question" title="问题"></vxe-table-column>
          <vxe-table-column field="answer" title="回答内容"></vxe-table-column>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="问题">
                <el-input size="small" type="textarea" placeholder="请输入问题" v-model="addForm.question"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="回答内容">
                <el-input size="small" type="textarea" placeholder="请输入回答内容" v-model="addForm.answer"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      this.loading = true;
      const res = await this.$api.problemlist({
    ...
    async toDel(row) {
      const res = await this.$api.del_problem({
        id: row.id
      })
      if (res.result == 1) {
        this.$message({
    ...
    AddOnSubmit(formName) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          if (this.isAdd) {
            // 新增
            const res = await this.$api.add_problem({
              ...this.addForm,
            });
            if (res.result == 1) {
              this.$message({
            ...
            // 修改
            const res = await this.$api.edit_problem({
              ...this.addForm,
              id:this.id
            });
            if (res.result == 1) {
