...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.post_rank();
      this.ruleForm.other_url = res.post_rank
    },
    // 提交
    async submitForm() {
      const res = await this.$api.editpost_rank({
        post_rank: this.ruleForm.other_url,
      });
      console.log(res);
      if (res.result == 1) {
        this.$message({
