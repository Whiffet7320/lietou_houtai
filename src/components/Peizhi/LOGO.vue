...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.app_logo();
      console.log(res);
      this.ruleForm.img = res.logo_pic
    },
    // 提交
    async submitForm() {
      const res = await this.$api.editapp_logo({
        logo_pic: this.ruleForm.img,
      });
      console.log(res);
      if (res.result == 1) {
        this.$message({
