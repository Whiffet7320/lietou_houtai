...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.com_enterpriseagreement();
      console.log(res);
    ...
    async onSubmitForm() {
      ...
      const res = await this.$api.editcom_enterpriseagreement(
        {
  ...
  mounted() {
    ...
    this.$api.aliyun_osssts().then(res => {
      let client = new window.OSS.Wrapper({
