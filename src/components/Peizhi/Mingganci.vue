...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.vocabulary();
      this.ruleForm.other_url = res.vocabulary
    },
    // 提交
    async submitForm() {
      const res = await this.$api.editvocabulary({
        vocabulary: this.ruleForm.other_url,
      });
      console.log(res);
      if (res.result == 1) {
        this.$message({
