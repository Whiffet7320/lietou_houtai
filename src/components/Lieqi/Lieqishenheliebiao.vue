<template>
  <div class="index">
      ...
      <div class="myTable">
        <vxe-table :loading="loading" align="center" :data="tableData">
          ...
          <vxe-table-column field="pic" title="营业执照">
            <template slot-scope="scope">
              <el-image v-for="(item,i) in scope.row.mylicense_pics" :key="i" :src="item" :preview-src-list="[item]" fit="fill"
                style="width: 40px; height: 40px;margin:0 4px">
                <div slot="error" class="image-slot">
                  <i class="el-icon-picture-outline"></i>
                </div>
          ...
          <vxe-table-column field="pic" title="人力资源许可证">
            <template slot-scope="scope">
              <el-image v-for="(item,i) in scope.row.myservicelicense_pics" :key="i" :src="item" :preview-src-list="[item]" fit="fill"
                style="width: 40px; height: 40px;margin:0 4px">
                <div slot="error" class="image-slot">
                  <i class="el-icon-picture-outline"></i>
                </div>
          ...
          <vxe-table-column title="操作" width="180">
            <template slot-scope="scope">
              <div class="flex">
                <el-button size="small" type="text" @click="toEdit(scope.row)">审核</el-button>
    ...
    <!-- 审核 -->
    <el-dialog title="审核" :visible.sync="addDialogVisible" width="700px" :before-close="addHandleClose">
      <div class="myAddForm">
        <el-form :model="addForm" :rules="rules" ref="addForm" label-width="100px" class="demo-addForm">
          ...
          <el-row v-if="addForm.status == 2">
            <el-col :span="20">
              <el-form-item label="审核理由">
                <el-input size="small" placeholder="请输入审核理由" v-model="addForm.reason"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      this.loading = true;
      const res = await this.$api.compayexaminelist({
